...
```{r, warning = FALSE}
cs <- read.spss("ZA6801_de_v4-0-1.sav", to.data.frame = TRUE)
```
...
## Psychometrics (cross-section data)
```{r}
cs <- cs %>%
  mutate(
    Openness = (as.numeric(q123e) + 6 - as.numeric(q123j)) / 2,
    Agreeableness = (as.numeric(q123b) + 6 - as.numeric(q123g)) / 2,
    Extraversion = (as.numeric(q123f) + 6 - as.numeric(q123a)) / 2,
    Conscientiousness = (as.numeric(q123c) + 6 - as.numeric(q123h)) / 2,
    Neuroticism = (as.numeric(q123i) + 6 - as.numeric(q123d)) / 2)

cs.psychometrics <- cs %>% select(Openness, Agreeableness, Extraversion, Conscientiousness, Neuroticism)

boxplot(cs.psychometrics)
rcorr(as.matrix(cs.psychometrics), type = "spearman")
plot_corheat(cs.psychometrics, method = "spearman")
```

## Psychometrics (panel data)
```{r}
panel <- panel %>%
...
## Climate policy attitudes (panel data)
```{r}
panel <- panel %>% 
...
panel <- panel %>%
  ...
  mutate(
    ...
    Female = as.numeric(kpx_2280.x) - 7,
    Age = 2019 - as.numeric(kpx_2290s.x %>% replace(kpx_2290s.x == "1955 und frueher", "1955"))) %>%
  # Replace exception codes with NA and merge responses from different waves, always keeping the newest
  mutate(
  ...
  mutate(
    LeftRight = ifelse(is.na(LeftRight12), LeftRight1, LeftRight12), # Only use wave 1 if no answer in wave 12
    Education = ifelse(is.na(School9), ifelse(is.na(School1), SchoolA1, School1), School9)) %>% # Use newest info on school education
  mutate(Education = Education %>% replace(as.numeric(kp1_2330) == 16 | as.numeric(kpa1_2330) == 16, 6)) # merge school & higher ed.
```
...
panel.final <- panel %>% select(
  ...
  Age, LeftRight, Female, Education)
```

# Generate descriptive and bivariate statistics
...
mod1.co <- lm(
  ...
  formula = 
    Priority ~ 
    Age + Female + LeftRight + Education)
summary(mod1.co)
```
...
```{r}
mod1.log.anova <- anova(mod1.log.co, mod1.log, test = "LRT")
mod1.log.anova
```
...
#### Likelihood ratio test
```{r}
mod2.log.anova <- anova(mod2.log.co, mod2.log, test = "LRT")
mod2.log.anova
```
...
#### Likelihood ratio test
```{r}
mod3.log.anova <- anova(mod3.log.co, mod3.log, test = "LRT")
mod3.log.anova
```
...
#### Likelihood ratio test
```{r}
mod4.log.anova <- anova(mod4.log.co, mod4.log, test = "LRT")
mod4.log.anova
```
